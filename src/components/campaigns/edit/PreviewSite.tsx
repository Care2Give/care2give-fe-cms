...
function buildPreviewLink(campaignState): URL {
  ...
  urlObj.searchParams.append(
    "targetDate",
    new Date(campaignState.endDate).toTimeString()
  );
  urlObj.searchParams.append("slug", campaignState.title);
  urlObj.searchParams.append("description", campaignState.description);
  const donationOptions: CampaignDonationAmount[] =
    campaignState.donationOptions;
  ...
  function Banner() {
    return (
      <div className="flex justify-end bg-slate-50">
        <Button className="m-2" onClick={() => setEditStage(EditStage.Publish)}>
          Close Preview
        </Button>
      </div>
    );
