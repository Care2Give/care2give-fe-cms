...
export function PublishDialog({setEditStage}) {
    ...
    const publishCampaign = () => {
        ...
        const bodyObj = {
            ...
            cents: campaignEditState.targetAmount - Math.floor(campaignEditState.targetAmount),
            editedBy: userId,
            // TODO change to not convert CampaignImage to just its URL when database is updated
            imageUrl: campaignEditState.images.map((campaignImage) => campaignImage.url),
        }
        ...
        getToken().then((token) => {
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/campaign/${isEdit ? campaignId : "create"}`, {
